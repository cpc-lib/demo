...
public class DateUtils extends org.apache.commons.lang3.time.DateUtils {
    ...
    public static Long getDaysDiff2(Date begin, Date end) {
    ...
    //计算收费的截止时间 getDeadLine的优化版本
    public static Date calculatePayDeadline(Date startDate, String day) throws ParseException {
...
//        n = 16-n;
    ...
    public static boolean isFirstDayOfMonth(Date date) {
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
        String original = DateUtils.parseDateToStr(DateUtils.YYYY_MM_DD, date);
        String yearMonth = DateUtils.parseDateToStr(DateUtils.YYYY_MM, date);
        String firstDayOfMonth = yearMonth + "-01";
        if (Objects.equals(original, firstDayOfMonth)) {
            return true;
        } else {
            return false;
        }
    ...
    public static Date getEnd(Date date) {
        LocalDateTime dateTime = DateUtils.convertDateToLocalDateTime(date);
        // 获取本月的最后一天
        LocalDateTime lastDayOfMonth = dateTime.with(TemporalAdjusters.lastDayOfMonth()).withHour(23).withMinute(59).withSecond(59);
        // 将LocalDateTime转换为Date
        Date ret = Date.from(lastDayOfMonth.atZone(ZoneId.systemDefault()).toInstant());
        return ret;
    }
    ...
    public static List<String> getLastNmonths(Integer nMonths) {
        List<String> months = new ArrayList<>();
        DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM");
        YearMonth current = YearMonth.now(); // 当前年月
    ...
    public static List<String> getLastNyears(Integer nYears) {
        int currentYear = Year.now().getValue(); // 当前年份，例如 2025
    ...
    public static List<String> getLastNQuarters(Integer nQuaters) {
        LocalDate now = LocalDate.now();
        int year = now.getYear();
        int quarter = (now.getMonthValue() - 1) / 3 + 1; // 当前季度
        List<String> quarters = new ArrayList<>();
        for (int i = 0; i < nQuaters; i++) { // 当前 + 过去4个
