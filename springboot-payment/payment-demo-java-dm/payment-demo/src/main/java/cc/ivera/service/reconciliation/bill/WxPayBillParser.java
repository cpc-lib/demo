...
@Slf4j
public class WxPayBillParser {

    private static final String HEADER_KEYWORD = "交易时间";
    ...
    public static List<ReconciliationDetail> parseTradeBillCsv(String csvContent, String batchNo) {
        ...
        for (String line : lines) {
            ...
            try {
                ...
                log.warn("解析微信账单行失败，跳过该行：{}，原因：{}", trimmed, e.getMessage());
            }
        }

        log.info("微信账单解析完成，共解析 {} 条交易记录，批次号：{}", result.size(), batchNo);
        return result;
    }
    ...
    private static ReconciliationDetail parseLine(String line, String batchNo) throws ParseException {
        ...
        detail.setOrderNo(removeQuotes(fields.get(6)));
        detail.setTransactionId(removeQuotes(fields.get(5)));
        detail.setChannelStatus(removeQuotes(fields.get(9)));
        detail.setChannelAmount(yuanToCents(removeQuotes(fields.get(12))));
