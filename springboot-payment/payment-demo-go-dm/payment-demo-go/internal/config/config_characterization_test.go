...
spring:
  ...
  datasource:
    ...
    url: jdbc:dm://dm.local:5236?schema=PAYMENT_DEMO_TEST&connectTimeout=30000
    username: SYSDBA
    password: db-pass
payment:
  order:
...
`)
	...
	if cfg.Database.Driver != "dm" {
		t.Fatalf("Database.Driver = %q, want dm", cfg.Database.Driver)
	}
	if cfg.Database.DSN != "dm://SYSDBA:db-pass@dm.local:5236?connectTimeout=30000&schema=PAYMENT_DEMO_TEST" {
...
func TestDatabaseDSN_CurrentBehaviorBuildsDamengDSNFromRawURL(t *testing.T) {
	got := databaseURLToDSN("dm://SYSDBA:SYSDBA@127.0.0.1:5236?schema=PAYMENT_DEMO", "ignored", "ignored")
	want := "dm://SYSDBA:SYSDBA@127.0.0.1:5236?schema=PAYMENT_DEMO"
	if got != want {
