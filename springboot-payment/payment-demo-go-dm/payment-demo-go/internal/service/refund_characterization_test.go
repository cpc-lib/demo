...
func TestRefundAutoQuery_CurrentBehaviorRetriesOnlyNonTerminalStatuses(t *testing.T) {
	for _, status := range []string{constant.RefundStatusCreated, constant.RefundStatusProcessing} {
		if !shouldRetryRefundAutoQuery(status, 1) {
			t.Fatalf("status %s should schedule another auto query", status)
		}
	}

	for _, status := range []string{constant.RefundStatusSuccess, constant.RefundStatusFailed, constant.RefundStatusClosed, constant.RefundStatusAbnormal} {
		if shouldRetryRefundAutoQuery(status, 1) {
			t.Fatalf("status %s should not schedule another auto query", status)
		}
	}

	if shouldRetryRefundAutoQuery(constant.RefundStatusProcessing, refundQueryMaxAttempts) {
		t.Fatalf("attempt %d should stop retrying", refundQueryMaxAttempts)
	}
}
