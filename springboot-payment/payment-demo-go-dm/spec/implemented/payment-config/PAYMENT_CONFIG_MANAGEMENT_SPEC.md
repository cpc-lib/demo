...
React/Vue 首页以支付应用作为唯一支付入口，不再单独选择支付方式；前端从选中应用的 `paymentType` / `channelCode` 推导下单接口。
...
- React/Vue 首页只展示可用支付应用，并从应用信息推导微信 V3、微信 V2 或支付宝下单入口。
...
| 2026-06-13 | implemented | 前端首页去掉独立支付方式选择，仅通过支付应用发起下单 |
