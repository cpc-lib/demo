...
- React/Vue 首页只展示可用支付应用，并把用户选择的应用 ID 传给下单接口；前端从支付应用自身的 `paymentType` / `channelCode` 推导调用微信或支付宝下单接口。
...
- [x] React/Vue 首页可直接选择支付应用；无可用应用时不能发起下单。
...
| 2026-06-13 | implemented | 首页去掉独立支付方式选择，改为从支付应用推导下单渠道 |
