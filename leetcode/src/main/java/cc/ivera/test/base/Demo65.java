...
public class Demo65 {
    ...
    public void test1() {
        ...
        List<User2> userList = userService2.getUserList();
        FastExcel.write(absolutePath, User2.class).sheet("用户信息").doWrite(userList);
    }
    ...
    public void test2() {
        ...
        String absolutePath = classPathResource.getFile().getAbsolutePath();
        // 创建ExcelWriter对象
        ExcelWriter excelWriter = FastExcel.write(absolutePath, User2.class).build();
        // 创建Sheet对象
        WriteSheet writeSheet = FastExcel.writerSheet("用户信息").build();
        // 向Excel中写入数据
        excelWriter.write(userService2.getUserList(), writeSheet);
        // 关闭流
        excelWriter.finish();
    }
    ...
    public void test3() {
        ...
        FastExcel.write(absolutePath, User2.class).excludeColumnFiledNames(excludeFields).sheet("用户信息").doWrite(userService2.getUserList());
    }
    ...
    public void test4() {
        ...
        // 写Excel
        FastExcel.write(absolutePath, User2.class).includeColumnFiledNames(includeFields).sheet("用户信息").doWrite(userService2.getUserList());
    }
    ...
    public void test5() {
        ...
        String absolutePath = classPathResource.getFile().getAbsolutePath();
        // 向Excel中写入数据
        FastExcel.write(absolutePath, User2.class).sheet("用户信息").doWrite(userService2.getUserList());
    }
     ...
     */
    @Test
    public void test9() throws IOException, FileNotFoundException, MalformedURLException {
    ...
    public void test13() {
        ...
        String absolutePath = classPathResource.getFile().getAbsolutePath();
        // 读取excel
        EasyExcel.read(absolutePath, User2.class, new AnalysisEventListener<User2>() {
            // 每解析一行数据,该方法会被调用一次
            @Override
            public void invoke(User2 demoData, AnalysisContext analysisContext) {
                System.out.println("解析数据为:" + demoData.toString());
            }
    ...
    public void test15() {
        ...
        String absolutePath = classPathResource.getFile().getAbsolutePath();
        // 创建ExcelReader对象
        ExcelReader excelReader = EasyExcel.read(absolutePath, User2.class, new AnalysisEventListener<User2>() {
            // 每解析一行数据,该方法会被调用一次
            @Override
            public void invoke(User2 demoData, AnalysisContext analysisContext) {
                System.out.println("解析数据为:" + demoData.toString());
    ...
    public void test16() {
        ...
        // 构建数据
        List<User2> userList = userService2.getUserList();
        User2 user2 = userList.get(0);
        // 填充excel 单组数据填充
        FastExcel.write(outputPath).withTemplate(absolutePath).sheet(0).doFill(user2);
    }
    ...
    public void test22() throws IOException {
        ...
        // 根据哪个模板进行填充
        ClassPathResource classPathResource = new ClassPathResource("easyexcel/model/test6.xlsx");
        ...
        // 构建数据
        List<User2> totalUsers = userService2.getUserList();

        //获取到分页后的excel文档
        List<User2Excel> user2Excels = User2Excel(totalUsers, 100);

        XSSFWorkbook workbook = new XSSFWorkbook(stream);


        workbook.setSheetName(0, user2Excels.get(0).getSheetName());
        ...
        for (int pageNum = 1; pageNum < totalPages; pageNum++) {
            //进行sheet克隆处理
            workbook.cloneSheet(0, user2Excels.get(pageNum).getSheetName());
        }
        ...
        for (User2Excel user2Excel : user2Excels) {
            WriteSheet writeSheet = FastExcel.writerSheet(user2Excel.getSheetName()).build();
            FillConfig fillConfig = FillConfig.builder().forceNewRow(true).direction(WriteDirectionEnum.VERTICAL).build();
            excelWriter.fill(user2Excel, writeSheet);
            excelWriter.fill(new FillWrapper("user2", user2Excel.getItemList()), fillConfig, writeSheet);
        }
        //手动关闭
        excelWriter.finish();
        //释放资源
    ...
    private List<User2Excel> User2Excel(List<User2> totalUsers, Integer pageSize) {
        List<User2Excel> user2Excels = new ArrayList<>();
        Integer totalPages = getPageNum(totalUsers, pageSize);
        for (Integer i = 0; i < totalPages; i++) {
            List<User2> pageData = getPageData(totalUsers, i, pageSize);
            List<User2Excel.User2> itemList = new ArrayList<>();
            for (User2 pageDatum : pageData) {
                User2Excel.User2 item = new User2Excel.User2();
                BeanUtils.copyProperties(pageDatum, item);
                itemList.add(item);
            }
            User2Excel user2Excel = new User2Excel();
            user2Excel.setSheetName("sheet" + (i + 1));
            user2Excel.setItemList(itemList);
            user2Excels.add(user2Excel);
        }
        return user2Excels;
    }


    private static void pageWrite(ExcelWriter excelWriter, List<User2> totalUsers, Integer pageNum, Integer pageSize, FillConfig fillConfig, WriteSheet writeSheet) {
        List<User2> users = getPageData(totalUsers, pageNum, pageSize);
        // 多组填充excel
        excelWriter.fill(new FillWrapper(users), fillConfig, writeSheet);
    }

    private static List<User2> getPageData(List<User2> totalUsers, Integer pageNum, Integer pageSize) {
        List<User2> page = totalUsers.stream().skip(pageNum * pageSize).limit(pageSize).collect(Collectors.toList());
        return page;
    }

    private static Integer getPageNum(List<User2> totalUsers, Integer pageSize) {
