...
export default function Home() {
  ...
  useEffect(() => {
    paymentConfigApi.apps().then((response) => {
      setPaymentApps(response?.data?.apps || [])
    })
  }, [])

  useEffect(() => {
    setPayOrder((prev) => {
      const apps = paymentApps.filter((app) => app.enabled && app.channelCode === prev.payType)
      if (apps.some((app) => app.id === prev.paymentAppId)) {
        return prev
      }
      return { ...prev, paymentAppId: apps.length > 0 ? apps[0].id : '' }
    })
  }, [paymentApps, payOrder.payType])
  ...
  const currentPaymentApps = paymentApps.filter((app) => app.enabled && app.channelCode === payOrder.payType)
      ...
      <section id="index" className="container">
        ...
        <div className="PaymentApp_payment-app-panel">
          ...
          <div className="PaymentApp_app-options">
            {currentPaymentApps.length > 0 ? currentPaymentApps.map((app) => (
              <div
                key={app.id}
              ...
              >
                ...
                <div className="PaymentApp_app-code">{app.appCode} · ID {app.id}</div>
              </div>
            )) : (
