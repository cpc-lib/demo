...
func (s *Service) CreateOrReuseOrder(ctx context.Context, productID int64, paymentType string) (*model.OrderInfo, error) {
	return s.CreateOrReuseOrderWithApp(ctx, productID, paymentType, 0)
}

func (s *Service) CreateOrReuseOrderWithApp(ctx context.Context, productID int64, paymentType string, paymentAppID int64) (*model.OrderInfo, error) {
	if productID <= 0 {
	...
	if paymentAppID <= 0 {
		return nil, util.Biz("paymentAppId不能为空")
	}
	lockKey := "payment:order:create:" + i64(productID) + ":" + paymentType
	if paymentAppID > 0 {
		lockKey += ":" + i64(paymentAppID)
	}
	v, err := s.Lock.ExecuteValue(ctx, lockKey, 3*time.Second, 10*time.Second, func() (interface{}, error) {
		...
		err := s.DB.Transaction(func(tx *gorm.DB) error {
			...
			query := tx.Clauses(clause.Locking{Strength: "UPDATE"}).Where("product_id=? and payment_type=? and order_status=?", productID, paymentType, constant.OrderStatusNotPay)
			query = query.Where("payment_app_id=?", paymentAppID)
			err := query.Order("create_time desc,id desc").First(&noPay).Error
			if err == nil {
				order = &noPay
				return nil
			}
			...
			if paymentAppID > 0 {
				newOrder.PaymentAppID = &paymentAppID
			}
