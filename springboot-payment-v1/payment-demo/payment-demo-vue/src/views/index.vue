      ...
      <div class="PaymentApp_payment-app-panel">
        ...
        <div class="PaymentApp_app-options">
          ...
          >
            ...
            <div class="PaymentApp_app-code">{{ app.appCode }} · ID {{ app.id }}</div>
          </div>
...
export default {
  ...
  created() {
    ...
    paymentConfigApi.apps().then((response) => {
      this.paymentApps = response.data.apps || []
  ...
  computed: {
    currentPaymentApps() {
      return this.paymentApps.filter((app) => app.enabled && app.channelCode === this.payOrder.payType)
    },
  ...
  methods: {
    ...
    selectDefaultPaymentApp() {
      const apps = this.paymentApps.filter((app) => app.enabled && app.channelCode === this.payOrder.payType)
    ...
    toPayV2() {
      ...
      if (this.payOrder.payType !== 'wxpay') {
        this.$message.warning('微信 V2 支付只支持微信支付方式')
        return
      }
      //禁用按钮，防止重复提交
      this.payBtnDisabled = true
