...
| 入口 | 类型 | 当前调用方 | 备注 |
|---|---|---|---|
| `cmd/server/main.go` | 进程入口 | 本地/运维启动 | 初始化配置、达梦数据库、Redis、RabbitMQ、支付客户端和 Gin 路由 |
...
| `payment.refund.query.release.queue` | RabbitMQ | 退款自动查询消费者 | 退款审核通过后延迟查询渠道退款结果 |
...
| Refund flow | `internal/service/refund_application.go`、`internal/service/refund_auto_query.go`、`internal/service/refund_info.go`、`internal/service/refund_status.go`、`internal/service/refund_status_mapping.go`、`internal/service/wx_refund.go` | 退款申请、审核、渠道提交、自动查询、状态同步、订单退款状态汇总 |
| Payment audit | `internal/service/payment.go` | 写入支付流水，重复流水按幂等成功处理 |
| MQ | `internal/mq/order_close.go`、`internal/mq/refund_query.go`、`internal/service/order_close.go` | 延迟关单和退款自动查询消息声明、发送、消费 |
...
- 现状：React/Vue 首页只展示可用支付应用，不再单独展示支付方式；前端从选中应用的 `paymentType` / `channelCode` 推导微信或支付宝下单接口。
...
退款审核通过并成功提交渠道后，会投递 `payment.refund.query.delay.queue` 延迟消息；消费者从 `payment.refund.query.release.queue` 调用单笔退款查询同步状态。`CREATED` / `PROCESSING` 会继续延迟重试，最多 5 次；`SUCCESS`、`FAILED`、`CLOSED`、`ABNORMAL` 不再自动重试。
...
| RabbitMQ | `payment.refund.query.event.exchange` | RabbitMQ | 退款审核通过 | 持久消息 | 退款自动查询生产入口 |
| RabbitMQ | `payment.refund.query.delay.queue` | RabbitMQ | 退款审核通过、自动查询重试 | TTL 队列 | TTL 为 60 秒 |
| RabbitMQ | `payment.refund.query.release.queue` | 消费者 | 死信交换机 | 长期 | 退款自动查询消费入口 |
...
- 首页下单入口以支付应用为唯一选择源，订单页仍展示订单持久化的支付方式。
...
| `payment-demo-go/internal/mq/order_close_characterization_test.go` | 延迟关单和退款自动查询事件名称、消息 JSON 格式 |
...
- **需要观察的数据**：订单状态、支付流水、退款状态、Redis 通知 key、RabbitMQ 延迟关单队列、退款自动查询队列。
...
| 2026-06-13 | implemented | 新增退款审核通过后的 RabbitMQ 延迟自动查询和重试兜底 | 退款审核后自动查询 |
| 2026-06-13 | implemented | 首页去掉独立支付方式选择，改为只选择支付应用并推导下单渠道 | 支付应用单选入口 |
