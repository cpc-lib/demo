package cc.ivera.grpcclient.demo.service;

import com.alibaba.fastjson2.JSONArray;
import com.alibaba.fastjson2.JSONObject;
import cc.ivera.grpcclient.demo.model.IdentityModel;
import cc.ivera.grpcclient.demo.model.PersonInfoModel;
import cc.ivera.grpcprotocol.*;
...
import static cc.ivera.grpcclient.demo.util.ParamUtils.getInt;
import static cc.ivera.grpcclient.demo.util.ParamUtils.getString;

@Service
public class ClientConsumer {
    @GrpcClient("grpc-server")
    //xxxBlockingStub用于简单grpc、服务端流式grpc
    private RemoteServiceGrpc.RemoteServiceBlockingStub stub1;

    @GrpcClient("grpc-server")
    //xxxServiceStub用于客户端流式grpc、双向流grpc
    private RemoteServiceGrpc.RemoteServiceStub stub2;
    ...
    public JSONObject queryInfo(PersonInfoModel customerListModel) {
        ...
        QueryInfoResponse response = stub1.queryInfo(request);
    ...
    public JSONObject querySomeData(int id) {
        ...
        JSONObject json = new JSONObject();
        //流式服务端返回的多个结果要用迭代器接收
        Iterator<QuerySomeDataResponse> iteraResponse = stub1.querySomeData(requestBuild.build());
    ...
    public JSONObject sendBooksInfo(int id) {
        ...
        StreamObserver<SendBooksInfoRequest> sendBooksStreamObserver = stub2.sendBooksInfo(responseStreamObserver);
        for (int i = 1; i < 4; i++) {
    ...
    public JSONObject calculateSum(int id) {
        ...
        StreamObserver<CalculateSumRequest> calculateSumStreamObserver = stub2.calculateSum(responseStreamObserver);
