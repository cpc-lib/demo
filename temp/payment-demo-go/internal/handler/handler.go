...
func (h *Handler) wxNativePay(c *gin.Context) {
	...
	data, err := h.Svc.WxNativePay(c.Request.Context(), productID)
	if err != nil {
		fail(c, err)
		return
	}
...
func (h *Handler) wxNativePayV2(c *gin.Context) {
	...
	data, err := h.Svc.WxNativePayV2(c.Request.Context(), productID, c.ClientIP())
	if err != nil {
		fail(c, err)
		return
	}
...
func (h *Handler) wxNativeNotify(c *gin.Context) {
	h.handleWxV3Notify(c, h.Svc.WxProcessOrderNotify)
}

func (h *Handler) wxRefundNotify(c *gin.Context) {
	h.handleWxV3Notify(c, h.Svc.WxProcessRefundNotify)
}

func (h *Handler) handleWxV3Notify(c *gin.Context, processor func(context.Context, map[string]interface{}) error) {
	...
	if err := h.Svc.WxV3.VerifyNotification(c.Request.Context(), c.Request.Header, body); err != nil {
		h.releaseNotify(c.Request.Context(), wxV3NotifyKeyPrefix, notifyID)
		wxV3Fail(c, "失败")
		return
	}
	if err := processor(c.Request.Context(), bodyMap); err != nil {
		h.releaseNotify(c.Request.Context(), wxV3NotifyKeyPrefix, notifyID)
		wxV3Fail(c, "失败")
		return
	}
...
func (h *Handler) wxV2Notify(c *gin.Context) {
	...
	if !pay.VerifyV2Sign(notifyMap, h.Svc.Cfg.WxPay.PartnerKey) {
	...
	if err := h.Svc.WxProcessV2Notify(c.Request.Context(), notifyMap, string(body)); err != nil {
		h.releaseNotify(c.Request.Context(), wxV2NotifyKeyPrefix, transactionID)
		c.String(http.StatusOK, wxV2Response("FAIL", "失败"))
		return
	}
...
func (h *Handler) aliTradePagePay(c *gin.Context) {
	...
	form, err := h.Svc.AliTradeCreate(c.Request.Context(), productID)
	if err != nil {
		fail(c, err)
		return
	}
...
func (h *Handler) aliTradeNotify(c *gin.Context) {
	...
	result := "failure"
	if !h.Svc.AliPay.VerifyNotify(c.Request.PostForm) {
	...
	if params["seller_id"] != h.Svc.Cfg.AliPay.SellerID {
		c.String(http.StatusOK, result)
		return
	}
	if params["app_id"] != h.Svc.Cfg.AliPay.AppID {
		c.String(http.StatusOK, result)
		return
	}
	...
	if err := h.Svc.AliProcessOrder(c.Request.Context(), params); err != nil {
		c.String(http.StatusOK, result)
		return
	}
