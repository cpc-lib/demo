...
### 1. AOP + 注解方式记录变更日志
- 使用 `@ChangeLog` 注解声明审计日志能力
- 使用 `ChangeLogAspect` 切面统一采集快照并入库
- Service 不再直接拼装日志，业务逻辑和审计逻辑解耦
- 已支持三类操作：`CREATE`、`UPDATE`、`DELETE`
- 支持两种字段配置模式：
...
## 本次增强
- 新增日志：创建成功后按 `#result` 取业务主键，回查新数据快照并记录 `CREATE` 日志
- 删除日志：删除前采集旧快照，删除成功后记录 `DELETE` 日志
- 更新日志：继续保留原有 `UPDATE` 能力
- 新增接口：`DELETE /userProfile/{id}`
- 原新增接口 `/userProfile/add`、`/userProfile/custom` 已自动写入创建日志
