...
public class ChangeLogAspect {
    ...
    public Object around(ProceedingJoinPoint joinPoint, ChangeLog changeLog) throws Throwable {
        ...
        String operationType = normalizeOperationType(changeLog.operationType());
        ChangeLogSnapshotProvider provider = findProvider(changeLog.entityClass());

        Object before = null;
        Object beforeCopy = null;
        Object bizIdBefore = null;
        if (requiresBeforeSnapshot(operationType)) {
            bizIdBefore = evaluateExpression(changeLog.bizId(), context);
            before = provider.loadById(bizIdBefore);
            beforeCopy = cloneObject(before, changeLog.entityClass());
        }
        ...
        Object bizIdAfter = evaluateExpression(changeLog.bizId(), context);
        String bizId = bizIdAfter != null ? String.valueOf(bizIdAfter)
                : (bizIdBefore != null ? String.valueOf(bizIdBefore) : null);
        if (bizId == null || bizId.isBlank()) {
            log.warn("变更日志记录跳过，bizId 为空, method={}, operationType={}", method.getName(), operationType);
            return result;
        }

        Object after = null;
        if (requiresAfterSnapshot(operationType)) {
            after = provider.loadById(bizIdAfter);
            if (after == null) {
                log.warn("变更日志记录跳过，操作后对象不存在, bizType={}, bizId={}, operationType={}",
                        changeLog.bizType(), bizId, operationType);
                return result;
            }
        }
    ...
    private boolean requiresBeforeSnapshot(String operationType) {
        return OP_UPDATE.equals(operationType) || OP_DELETE.equals(operationType);
    }

    private boolean requiresAfterSnapshot(String operationType) {
        return OP_UPDATE.equals(operationType) || OP_CREATE.equals(operationType);
    }
