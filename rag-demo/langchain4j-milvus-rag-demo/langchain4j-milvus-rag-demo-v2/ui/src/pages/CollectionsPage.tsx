...
export default function CollectionsPage() {
  ...
  const describeCollection = async (collectionName: string) => {
    try {
      const { data } = await ragApi.describeCollection(collectionName, databaseName);
      setDescribeResult(data.data);
      message.success(`已读取 ${collectionName} 详情`);
    } catch (error) {
      message.error(getErrorMessage(error));
    }
  };
  ...
  return (
    <Space direction="vertical" size={16} style={{ width: '100%' }}>
      ...
      <Card className="page-card" bordered={false} title="集合列表">
        <Table
          ...
          pagination={false}
          columns={[
            { title: '集合名', dataIndex: 'collectionName', render: (v, r: any) => v || r.name || '-' },
            { title: '描述', dataIndex: 'description', render: (v) => v || '-' },
            {
              title: '操作',
              render: (_, record: any) => (
                <Button size="small" type="link" onClick={() => describeCollection(record.collectionName || record.name)}>
                  查看详情
                </Button>
              )
            }
