...
export default function VectorStoresPage() {
  ...
  const loadData = async () => {
    try {
      ...
      setStores(Array.isArray(listRes.data) ? listRes.data : []);
    } catch (error) {
  ...
  return (
    <Space direction="vertical" size={16} style={{ width: '100%' }}>
      ...
      <Row gutter={[16, 16]}>
        <Col xs={24} xl={10}>
          <Card className="page-card" bordered={false} title="新增 / 更新配置">
        ...
        <Col xs={24} xl={14}>
          <Card className="page-card" bordered={false} title="当前生效配置" loading={loading}>
            {current ? (
          ...
          <Card className="page-card" bordered={false} title="全部向量库配置" style={{ marginTop: 16 }}>
            <Table
              loading={loading}
              rowKey="alias"
              dataSource={stores}
