...
//        Files.delete(Paths.get("D:\\Snipaste-1.16.2-x64 - 副本"));
        //deleteFiles("D:\\develop\\jar-backup");
        Path rootPath = Paths.get("D:\\develop\\workspace\\document");
        // 调用函数删除空目录
        ...
        try (DirectoryStream<Path> stream = Files.newDirectoryStream(path)) {
            for (Path filePath : stream) {
                if (Files.isDirectory(filePath)) {
                    deleteEmptyDirectories(filePath); // 递归调用以处理子目录
                    // 如果目录为空，则删除
                    if (Files.list(filePath).count() == 0) {
                        Files.delete(filePath);
                        System.out.println("Deleted empty directory: " + filePath);
                    }
                }
            }
        } catch (IOException e) {
