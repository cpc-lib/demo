...
-- 快速造 100 万测试数据：递归 CTE 生成 0~999，再三表笛卡尔积
...
SELECT
  a.n * 1000000 + b.n * 1000 + c.n + 1 AS id,
  CONCAT('user_', a.n * 1000000 + b.n * 1000 + c.n + 1),
  18 + ((a.n + b.n + c.n) % 50),
  CONCAT('13', LPAD((a.n * 1000000 + b.n * 1000 + c.n + 1) % 1000000000, 9, '0')),
  CONCAT('user', a.n * 1000000 + b.n * 1000 + c.n + 1, '@example.com'),
  NOW() - INTERVAL ((a.n + b.n + c.n) % 365) DAY,
  NOW()
FROM (SELECT n FROM seq WHERE n = 0) a
JOIN seq b
JOIN seq c;
