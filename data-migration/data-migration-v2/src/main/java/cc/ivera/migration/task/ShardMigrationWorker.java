...
public class ShardMigrationWorker implements Runnable {
    ...
    private void doRun(int shardNo, long startId, long endId, int retry) {
        ...
        log("INFO", "SHARD", "开始分片 shardNo=" + shardNo + ", keysetRange=" + startId + "~" + endId + ", idColumn=" + idColumn);
        ...
        while (true) {
            ...
            try {
                ...
                insertBatchLog(shardNo, batchNo, batchStart, batchEnd, selectedRows, cost, tps, MigrationStatus.SUCCESS, null);
                lastCursor = batchEnd;
                batchNo++;
            } catch (Exception e) {
                long cost = Math.max(System.currentTimeMillis() - begin, 1);
     ...
     */
    private void migrateBatch(long startId, long endId) throws DataAccessException {
