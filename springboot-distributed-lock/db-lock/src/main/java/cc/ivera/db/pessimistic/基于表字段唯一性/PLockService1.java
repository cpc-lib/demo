package cc.ivera.db.pessimistic.基于表字段唯一性;

import cc.ivera.db.dao.entity.CommodityStock;
import cc.ivera.db.dao.entity.ResourceLock;
import cc.ivera.db.service.CommodityStockService;
import cc.ivera.db.service.ResourceLockService;
...
public class PLockService1 {
    ...
    public boolean commodityReduce(String commodityCode) throws Exception {
        ...
        //synchronized (PLockService1.class)会锁住事务
        {
            // 首先要插入
            boolean bSave = false;
            ...
            while (!bSave) {
                try {
                    ...
                    System.err.println(Thread.currentThread().getName() + ":" + port + ",获取锁失败...," + e.getMessage());
                }
                i++;
                Thread.sleep(1);
                if (i == 5) {
