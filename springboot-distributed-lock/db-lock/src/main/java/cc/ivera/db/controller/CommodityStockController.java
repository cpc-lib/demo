package cc.ivera.db.controller;


import cc.ivera.common.result.Result;
import cc.ivera.db.nolock.NoLockService;
import cc.ivera.db.optimistic.OLockService;
import cc.ivera.db.pessimistic.基于表记录行锁.PLockService2;
...
public class CommodityStockController {
    ...
    public Result oLockCommodityReduce(@PathVariable("commodityCode") String commodityCode)
        ...
        if(bRes){
            System.out.println(Thread.currentThread().getName()+"减库存成功！0");
            return Result.ok("减库存成功！");
        }else{
            System.out.println(Thread.currentThread().getName()+"减库存失败！-1");
            return Result.ok("减库存失败！");
        }
    }
